    ...
    internal static class ToolHelpCatalog
    {
        ...
        private static readonly Dictionary<string, string> _helpTexts = new(System.StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["genexus_module"] =
                ...
                "- `install` / `install_builtin` — install a local `.opc` (`opcFile`) or resolve `name`/optional exact `version` from the installed SDK's built-in packages. Arbitrary remote named installation is not supported by this route.\n" +
