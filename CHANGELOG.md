...
- **`genexus_worker_reload` no longer accepts an unverified forced hard hot-swap ([#215](https://github.com/lennix1337/Genexus18MCP/issues/215)).** `force=true` with `mode=hard` is rejected before any Worker is stopped because the direct path cannot apply `sourceDir`; graceful `mode=hard` remains the verified drain-window copy path. Forced reload responses now declare `scope=all-open-workers` and `selectorApplied=false`, and the schema/help mark the global operation as destructive.
