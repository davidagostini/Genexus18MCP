    ...
    internal static class ToolHelpCatalog
    {
        private static readonly Dictionary<string, string> _helpTexts = new(System.StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["genexus_edit"] =
                ...
                "Source and Rules are always re-read after the single SDK save. For WebPanel `part=Events`, a text patch uses the native part-only save boundary: it writes the ISource part and commits its transaction without serializing or saving the whole WebPanel; PatternInstance is never used for that path. `verifyMode: 'normalized'` is the default and tolerates EOL, encoding marker, trailing-whitespace, and repeated-blank-line rendering by the SDK; `exact` preserves comments, whitespace, and blank lines but treats CRLF/LF as the same logical Source representation; `semantic` also tolerates harmless SDK casing/spacing changes. Comment-only Replace writes require `baseVersion`, are verified against the requested comment, report active old-statement presence, and return `CommentOnlyWriteNotPersisted` if the SDK re-read diverges. The response separates `saved` from `verified` and includes raw/normalized hashes, `normalizationApplied`, `diffNormalized`, `matchCount`, `persistedMatchCount`, `oldContentPresent`, `replacementPresent`, `reReadConfirmed`, and `implicitOperations` (always empty for this path). A mismatch is never reported as Applied. Rollback occurs only with `rollbackOnFailure: true` and a valid snapshot, and reports its own save/verification hashes; `.bak.gz` Events snapshots are decompressed and restored as Events. Pass the prior read's `versionToken` as `baseVersion` to reject concurrent edits. No Specify, Generate, Build, Rebuild, compilation, reorganization, execution, or tests are invoked by a patch write.\n\n" +
                "## Disambiguation\n" +
