...
# TOKENS
Defines reusable tokens grouped by category and optionally scoped by argument value
...
- Same concept as CSS-variables with GeneXus-specific syntax
...
Rules:
- See [GeneXus Style Properties](./frontend-styles.md) for available GeneXus-specific properties
- Web accepts full CSS selector/function capabilities; mobile supports restricted subset
- All token references use `$<token-group>.<token-name>` syntax
...
- Keep only classes referenced by `Panel` layouts or other classes
- Bind only one `DesignSystem` object with each `Panel` object
	* Use `@import` rules for shared definitions
	* Add common definitions in imported `DesignSystem` objects
