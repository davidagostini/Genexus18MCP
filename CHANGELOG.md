...
- **A stdio gateway now exits cleanly when its input stream reaches EOF ([#219](https://github.com/lennix1337/Genexus18MCP/issues/219)).** Closing the MCP client's stdin no longer leaves a gateway, worker, and ownership lease running indefinitely when an HTTP port is configured. The gateway drains in-flight stdio requests for a bounded period, cancels its HTTP lifetime, stops the worker, and releases the lease; HTTP-only gateways remain long-lived. This prevents orphaned processes and misleading cold-start/index diagnostics for the next session.
