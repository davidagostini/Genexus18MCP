# Plan 091: Fence multi-target async mutation retries
...
`Program.RequestLoop.cs:2464-2468,2550-2554,2639-2645` records and confirms only `name`; `targets[]` can leave it blank. `MutationRecoveryRegistry.cs:45-47` silently returns for blank targets.

Represent every target deterministically, create per-target/part fences or a documented aggregate fence, and clear the fence only after all required reads succeed. Preserve operation identity and idempotency semantics. Add watchdog/stall, partial completion, retry, and all-target read-back tests.

Scope: recovery identity, async edit path, tests, envelopes if necessary. Out of scope: unrelated mutation semantics.

Done: multi-target timeout cannot retry without reconciliation; complete read-back clears all fences; single-target compatibility remains green.
