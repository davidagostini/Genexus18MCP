...
- **Generic pattern apply verifies the resolved instance before reporting success.** Resolution checks registered pattern type and ownership, so an untyped homonym cannot produce a false `PatternApplied`; `reapply=true` may infer `pattern` from the existing instance. Unsupported headless reapply returns `PatternRouteUnsupported` ([#260](https://github.com/lennix1337/Genexus18MCP/issues/260)).
