# Plan 089: Sanitize client-visible infrastructure errors
...
Raw exception text is returned at `src/GxMcp.Gateway/Program.Http.cs:560-563`, `src/GxMcp.Gateway/KbImportHelper.cs:112-119`, `src/GxMcp.Gateway/MacroSuggestionService.cs:146-149`, and `src/GxMcp.Worker/Services/PreviewService.cs:826-839`.

Introduce stable error codes/messages for clients, retain details only in protected structured logs, and add correlation/operation identifiers without logging secrets. Update contract tests to assert no raw path/exception text is present while diagnostics remain available to maintainers.

Scope: cited error adapters and conformance tests. Out of scope: removing useful typed domain errors.

Done: each cited path has sanitized output, protected logging, and regression coverage.
