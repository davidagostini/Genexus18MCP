# Plan 093: Bound Worker MTA command concurrency
...
`src/GxMcp.Worker/Program.cs:455-469` uses `Task.Run` for every thread-safe command; the input queue bounds lines but not active tasks.

Add a bounded concurrency gate/dedicated pool, preserve cancellation and health priority, and ensure shutdown waits/disposes cleanly. Add burst tests asserting active-task ceiling, terminal response uniqueness, and STA responsiveness.

Scope: Worker dispatch loop and tests. Out of scope: SDK STA admission policy.

Done: active MTA tasks never exceed configured bound; cancellation/health are not starved; full Worker suite passes.
