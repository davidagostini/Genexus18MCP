# Plan 087: Restrict preview artifact paths
...
`src/GxMcp.Worker/Services/PreviewService.cs:783-824` combines the configured baseline directory with request-derived `name` for `.png` and `.a11y.json` writes.

Implement logical-name validation (reject rooted paths, separators, traversal, invalid Windows filename characters), canonicalize the baseline root, and verify every final path is under that root before creating or writing. Preserve valid GeneXus identifiers and return a stable validation error without exposing unnecessary filesystem details.

Scope: `PreviewService.cs` and its focused tests. Out of scope: changing preview protocol or baseline storage layout.

Done: tests cover valid names, traversal, rooted paths, separators, and both screenshot/baseline writes; all verification commands pass.
