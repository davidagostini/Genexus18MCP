    ...
    internal static class ToolHelpCatalog
    {
        private static readonly Dictionary<string, string> _helpTexts = new(System.StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["genexus_edit"] =
                ...
                "Source and Rules are always re-read after the single SDK save. `verifyMode: 'normalized'` is the default and tolerates EOL, encoding marker, trailing-whitespace, and repeated-blank-line rendering by the SDK; use `exact` for raw-text identity or `semantic` for harmless SDK casing/spacing changes. The response includes raw/normalized hashes, `normalizationApplied`, `diffNormalized`, `matchCount`, `replacementPresent`, and `reReadConfirmed`. A mismatch returns `WriteNotPersisted` without retrying the write. Rollback occurs only with `rollbackOnFailure: true` and a valid snapshot. Pass the prior read's `versionToken` as `baseVersion` to reject concurrent edits.\n\n" +
                "## Disambiguation\n" +
