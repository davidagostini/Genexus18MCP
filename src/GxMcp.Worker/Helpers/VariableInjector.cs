...
namespace GxMcp.Worker.Helpers
{
    public static class VariableInjector
    {
        ...
        // Compatibility overload for the typed-writer paths introduced before the
        // result-bearing binder. A failed native reference must never be ignored.
        ...
        {
            if (!BindVariableToDomain(v, domain, out var failure))
                throw new InvalidOperationException("VariableTypeNotPersisted: " + failure);
        }
