    ...
    internal static class ToolHelpCatalog
    {
        ...
        private static readonly Dictionary<string, string> _helpTexts = new(System.StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["genexus_wwp"] =
                ...
                "- `move_grid_column` — select exactly one `attribute` or `variable` in `gridPath`; `before` moves it immediately before another column, and optional `caption` updates its title. Omit `before` for caption-only edits.\n" +
                "- `add_grid_variable` — add a presentation `variable` column with `caption`, `basicType` (`Character`/`VarChar`) and `length` (1–9999); optional `before` selects insertion position. This does not change a table or author Grid.Load logic.\n" +
                "Both grid actions require a token from a fresh read as `expectedVersion` (aliases `baseVersion`/`versionToken`), even for `dryRun`. `gridPath` is an absolute element-type path with optional zero-based same-type sibling indexes. Hosts and WebComponents resolve by identity. Saves preserve native automatic Pattern projection and verify independent PatternInstance/WebForm rereads; ambiguous writes must never be retried automatically.\n" +
