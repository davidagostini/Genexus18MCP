# Plan 102: Replace per-start WMI scans with scoped ownership
...
`src/GxMcp.Gateway/WorkerProcess.cs:821-824` calls `KillOrphanWorkers` on every start; `:630-657,734-767` performs system-wide enumeration and WMI parent/command-line queries.

Design an explicit ownership registry, named synchronization primitive, or Windows Job Object; retain a narrowly scoped periodic reconciliation fallback for legacy/orphan cleanup. Prove cross-checkout isolation and failure behavior before removing scans.

Scope: WorkerProcess lifecycle and tests/design docs. Out of scope: broad process supervisor rewrite.

Done: startup/respawn avoids repeated global scans, owned workers are cleaned, unrelated workers survive, and fallback behavior is documented/tested.
