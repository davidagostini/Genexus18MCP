# Plan 105: Make GeneXus SDK compatibility explicit and reproducible
...
`src/GxMcp.Worker/GxMcp.Worker.csproj:20-120` references Artech/GeneXus assemblies through `GX_PATH` with `Private=False` and no SDK lock/fingerprint.

Define supported version/fingerprint checks at build and startup, report mismatch clearly, and document a self-hosted build image/fixture without copying proprietary SDK files or secrets. Add tests for match, mismatch, missing path, and diagnostic output.

Scope: project validation, startup diagnostics, CI/docs/tests. Out of scope: pretending the SDK is a NuGet dependency or redistributing it.

Done: unsupported SDKs fail early with actionable stable diagnostics; supported SDK builds remain green.
