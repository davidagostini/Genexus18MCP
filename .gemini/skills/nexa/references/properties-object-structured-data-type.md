...
# SDT ITEM SERIALZIATION

## Collection Serialization
XML serialization for collections
- Type: `enum{Wrapped;Sequence}`
- Options:
	* `Wrapped`: Collection wrapped by the collection name
	* `Sequence`: Plain sequence of collection items
- Default:  `Wrapped`
- Require: `Collection = True`

## Json Name
JSON field name used in serialization; uses the `Name` value when unspecified
- Type: `string``

## Json Null Serialization
JSON serialization of null values
- Type: `enum{Empty;JSON null;No Property}`
- Options:
	* `Empty`: Empty value or collection
	* `JSON null`: "JSON null value
	* `No Property`: Property omitted from JSON
- Default: `Empty`
- Availability:
	* ProductVersion: `>=15.8`

## Json Collection Serialization
JSON serialization for collection
- Type: `enum{Wrapped;Sequence}`
- Options:
	* `Wrapped`: Collection wrapped by the collection name
	* `Sequence`: Plain sequence of collection items
...
## Soap Type
SOAP XML type for serialization
- Type: `string`
- Notes:
	* Uses the default GeneXus XML type mapping when unspecified
	* Compatible types can serialize member contents as raw XML instead of an enclosing XML element
	* Use unrestricted XML types only when required; validate or sanitize untrusted XML contents

## XML Name
XML field name used in serialization; uses the `Name` value when unspecified
- Type: `string`

## XML Namespace
Custom XML namespace used in serialziation
- Type: `string`

## XML Null Serialization
XML serialization of null values
- Type: `enum{Empty Tag;No Tag;Nil Tag}`
- Options:
	* `Empty Tag`: Serialize always
	* `No Tag`: Omit when empty
	* `Nil Tag`: Serialize as xsi:nil
- Default: `Empty Tag`
- Availability:
	* ProductVersion: `>=12.2`

## XMLCollectionItemName
XML field name for collection item in serialziation
- Type: `string`
- Require: `Collection = true`