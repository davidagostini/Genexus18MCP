...
- **Hardened user-supplied file exchange paths.** Object part/blob export and import, Object Text batch exchange, XPZ transfer, API baseline comparison, and profiler XML ingestion now resolve quoted Windows paths relative to the active KB and reject traversal or absolute paths outside the active KB, configured GeneXus installation, or an explicit `GXMCP_EXTERNAL_IO_ROOT` staging root. This prevents read-only inspection tools from becoming arbitrary local-file readers and prevents export helpers from overwriting unrelated files. The new path policy is covered by traversal, quoted-path, external-root, and diagnostics tests.
- **Escaped the Indexes webview object title.** An object name from the workspace URI can no longer break the initial HTML document before the data payload is rendered.
...
- **Injected the file-path policy through the Worker composition root.** File-exchange services now depend on the small `IUserFilePathPolicy` port, while `CommandDispatcher` owns the concrete policy and shares one instance. Public constructors remain compatibility adapters for existing callers and tests.
