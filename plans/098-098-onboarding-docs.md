# Plan 098: Make onboarding prerequisites authoritative
...
`README.md:49,160` says Node 18+ while `package.json:52` requires Node 22+; `README.md:531-533` instructs nonexistent `.\setup.bat`; `CONTRIBUTING.md:11` and `build.ps1` are current references.

Choose package.json as source of truth, update prerequisite and install/build sections, replace or explicitly justify the obsolete setup command, and add a documentation check if practical. Keep Windows/GeneXus prerequisites explicit.

Scope: README and related onboarding docs/tests. Out of scope: changing package engine or creating an unrequested installer.

Done: every documented setup command exists and Node version statements agree.
