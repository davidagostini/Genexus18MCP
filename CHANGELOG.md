...
- **Explicit `kb` requests now keep their session-owned KB lease alive ([#225](https://github.com/lennix1337/Genexus18MCP/issues/225)).** Requests that resolve to the selected KB renew its lease without changing the context generation. Stateful requests with an explicit different or expired target adopt a fresh owner-bound context before the ownership fence runs, while stateless requests do not change session selection. This prevents long-running sessions from reaching `KB_LEASE_EXPIRED` after otherwise successful explicit-KB calls and does not perform any Knowledge Base mutation.
