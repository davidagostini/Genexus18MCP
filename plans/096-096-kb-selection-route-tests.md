# Plan 096: Cover end-to-end KB selection routes
...
`Program.RequestLoop.cs:789-972` contains route branches for `select`, `set_session_default`, `set_persistent_default`, and legacy `set_default`; existing store tests at `SessionKbContextStoreTests.cs:24-120` do not invoke `ProcessMcpRequest`.

Add stdio and session-bound HTTP route tests that issue actual `tools/call`, verify in-memory versus persisted behavior, `persistedTo`, independent session selections, and subsequent resolution. Use temporary config files and existing test seams; do not require the SDK.

Scope: Gateway route tests and minimal seams. Out of scope: changing resolution policy.

Done: route-level assertions cover success, invalid alias, session isolation, disk read-back, and legacy compatibility.
