...
- **`genexus_transfer action=import` now fails closed on hollow XPZ packages ([#221](https://github.com/lennix1337/Genexus18MCP/issues/221)).** The destructive path reads the XPZ metadata before `ImportFile` and refuses malformed, unnamed, object-less, or empty-part object records, preventing the SDK's permissive `IExportItem` projection from creating an empty object. `inspect` and `dryRun=true` now report package object names/types, part counts, payload presence, and `preflight.validForImport`. Rejections explicitly state that no import was attempted.
