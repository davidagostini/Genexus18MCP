...
- The Windows MCP smoke contract now invokes the repository-required `pwsh` host, retries only post-listener transport failures with a bounded budget, and includes smoke-host plus Gateway process/output diagnostics when the subprocess fails; HTTP contract responses are not retried ([#271](https://github.com/lennix1337/Genexus18MCP/issues/271)).

- `genexus_kb_version action=changed_objects` now filters candidate revisions by object `LastUpdate` and confirms content with the SDK comparer instead of treating the optimistic write token (including parent identity) as content equality. The response identifies the frozen baseline path and comparison semantics, and fails closed when the SDK cannot compare content ([#273](https://github.com/lennix1337/Genexus18MCP/issues/273)).

- **Large-KB index warm starts no longer discard the last usable delta baseline.** Every certified-slot promotion carries the previous conservative `meta.json` sidecar, interrupted lite walks checkpoint a resumable partial index, and per-KB publication locks prevent a lagging Worker from replacing a newer certified slot. Lifecycle status/whoami now expose the cache rejection reason, slot generation, checkpoint resume point, and background index activity ([#266](https://github.com/lennix1337/Genexus18MCP/issues/266)).
...
- **Index readiness now follows the post-alias action.** `genexus_db` drift/types actions and `genexus_versioning diff_generated` are gated when they depend on the index, while SQL, records, and other independent umbrella actions remain available during a build.

### Changed

- **`genexus_whoami` preserves `geneXus.catalog.entries` and `legacyEntries` in the default payload.** Existing clients may consume those fields; the internal slim catalog projection remains available only to explicit callers and is no longer silently substituted into the standard health contract.
...
- **`genexus_whoami` keeps the static version-catalog entry detail in the default health check for backward compatibility.** The per-major entry objects (`displayName`, `driver`, `defaultInstallPath`, `registryNames`, `legacyRegistryVersions`) and their legacy counterparts are derived from `config/gx-versions.json` and cannot change during a session, so they remain available to existing clients while explicit internal callers can request a slim projection. The identity fields the whoami contract exposes (`source`, `primaryMajor`, `supportedMajors`, `legacyMajors`) are unchanged, and verbose mode continues to add the optional playbooks, skills, and stats blocks. Measured on a real KB (GeneXus 18, `C:\kbs\KBTeste`): The previous lean-payload benchmark is not the standard contract. `genexus_whoami` advertises no `outputSchema`, so no strict client is affected; `GeneXusVersionCatalog.ToDiagnosticObject()` keeps the entry detail by default for the KB-provisioning caller that resolves a declared major to its install path.
- **The first-touch warm pass now warms `genexus_search_source`.** The first Source search after a Worker cold start builds the Worker's KB-wide source-scan cache on the STA thread, and that cost is pattern-independent — a zero-match pattern paid the same price as a matching one. It was the last SDK-heavy first touch still paid inside the agent's turn (every other one, `read` Structure/Source, `inspect`, `analyze` linter/callers, was already warmed). Measured on the same real KB: **first `genexus_search_source` after open 2717–4351 ms → 36 ms**, with the benchmarked p95 falling 7.63 → 2.59 ms because the warm pass also removes the cold first read from the measured population. The warm step is best-effort and individually guarded like the existing ones, and requests `maxResults=1` so its own reply stays tiny.

- **A session's first index-dependent read now returns data instead of a retry envelope.** Right after a KB open the gateway's index mirror held the snapshot restored from the warm cache (`status=Ready freshness=stale opState=Starting`) while the Worker's delta refresh republished `Freshness=current` a few hundred ms later. The read gate only re-probes a mirror older than its 2 s floor, so a cold session's first `genexus_list_objects`/`query`/`read` was answered with `IndexNotReady` (`retryAfterMs=5000`) against an index that was already usable, costing the agent a backoff and a wasted turn. The bootstrap now settles the mirror as soon as the delta lands, and an index-dependent call that arrives while that settle is in flight waits for it (bounded) rather than fast-failing; when the mirror reports that exact restored-snapshot state (`Ready` with a non-current freshness) the read gate settles it itself, so the outcome does not depend on winning a race with the bootstrap that publishes the settle. A genuinely cold KB is untouched: it has no restored snapshot, reports a streaming status instead of `Ready`, and keeps the immediate envelope plus `retryAfterMs`. Because the fix lives in the shared read gate, it covers every index-dependent entry point, not just listings — measured per tool on the same KB and client, `genexus_search_source` 9645 → 4881 ms, `genexus_inspect` 9403 → 5124 ms, `genexus_query` 9468 → 4824 ms, `genexus_analyze` 9514 → 5119 ms, `genexus_navigation` 9685 → 4748 ms, `genexus_security` 9623 → 4780 ms, `genexus_orient` 9528 → 4690 ms and `genexus_list_objects` 9851 → 4477 ms. All eight went from receiving a 478 B `IndexNotReady` envelope in 100% of sessions to real data with zero retries in 100% of sessions. Measured on a real KB (GeneXus 18, 620 objects) for a client that honors `retryAfterMs`: **first useful result 9851 → 4477 ms (-54.6%)** across repeated full sessions, with the first call returning the real listing (1503 B) and zero retries instead of a 478 B `IndexNotReady` envelope plus a 5 s backoff. Post-fix sessions landed in a 4.43–5.59 s band across 14 runs, tracking session-to-session variance in `KnowledgeBase.Open` (measured 1.66–2.57 s on this machine) rather than the gate — the Worker `[COLD-START]` total stayed within 3.58–4.05 s and retries were zero in every run. Even the slowest post-fix session is well under the fastest pre-fix one (9814 ms). The first call's own latency rises by ~250 ms while the settle completes; a genuinely cold KB keeps the immediate envelope, and the settle's budget is fixed and logged. The silent gate now logs `[IndexGate] closed` with the status/freshness/operation state and mirror age.
...
- **The index gate's tool set is now enumerable, with a guard against entries the legacy alias rewrite consumes.** `IsIndexDependentTool` used a switch, so nothing could assert what the gate actually protects. It now reads from a `HashSet` (same case-insensitive matching, O(1) lookup), and a test fails if a gate entry is a name the alias rewrite replaces before dispatch. That check found three such entries already in place — `genexus_db_drift`/`genexus_types` → `genexus_db` and `genexus_diff_generated` → `genexus_versioning` — whose umbrellas are not gated, so the read gate silently stopped protecting those paths when the tools were consolidated. Closing that needs per-action gating (the umbrellas mix index-dependent and independent actions), and regression tests now assert the action-level policy instead of recording a known bypass.
