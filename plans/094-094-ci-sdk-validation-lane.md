# Plan 094: Add executable GeneXus SDK CI validation lane
...
`/.github/workflows/ci.yml:111-135` accepts Gateway-only coverage when SDK is unavailable; `scripts/coverage/collect.ps1:103-121` documents the skip.

Define a protected self-hosted Windows lane with explicit SDK version/path checks, fixture setup/cleanup, live Worker tests, artifact retention, and license/machine preconditions. Keep hosted CI fast and make skipped SDK validation visible in release status. Do not put credentials or licensed artifacts in the repository.

Scope: workflow, scripts, docs, tests needed for deterministic lane. Out of scope: weakening hosted gates or publishing SDK binaries.

Done: workflow dry-run/schema validation passes; self-hosted lane has explicit pass/skip/fail states and cleanup.
