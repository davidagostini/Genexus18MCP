...
- For `<name>/` + `<name>.gx` sibling file → `Transaction` directory
- For `<name>/` + `module.toml` child file → `Module` directory
...
- `#patternsettings/`: Pattern-specific artifacts
...
- `src/Client/BookingPanel.panel.xml` with:
	* `<view platform="Any Platform">`
	* `<view platform="Any Phone">`
