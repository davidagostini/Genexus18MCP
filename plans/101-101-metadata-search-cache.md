# Plan 101: Reuse metadata resolution during source search
...
`src/GxMcp.Worker/Services/SourceSearchService.cs:532-577` resolves the same candidate object and parts repeatedly for requested metadata fields.

Resolve each candidate once within STA ownership, cache only request-local part values, preserve partial-index and cancellation semantics, and add multi-field timing/operation-count coverage.

Scope: SourceSearchService and focused tests/benchmark. Out of scope: changing search ranking or response schema.

Done: repeated SDK resolution is eliminated for one candidate; result equivalence and cancellation tests pass.
