...
- **`genexus_db action=sql_navigation` now parses GX17 navigation predicates from their semantic XML fields ([#224](https://github.com/lennix1337/Genexus18MCP/issues/224)).** Nested `Attribute/AttriName`, `Token`, and `Variable/VarName` nodes are read individually instead of using the parent element's concatenated inner text. Control-flow markers such as `NotEndOfTable` are excluded from SQL, preventing duplicated or malformed `WHERE` clauses and preserving the expected bind parameters.
