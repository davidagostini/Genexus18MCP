# Plan 092: Single-flight cold index loading
...
`src/GxMcp.Worker/Services/IndexCacheService.cs:1011-1019` starts `Task.Run(GetIndex)` without a load-in-progress guard while initialization also starts a load around `:553-555`.

Use a shared atomic load task/state so concurrent callers observe one load, reset state after failure, and preserve nonblocking `TryGetLoadedIndex`. Add concurrent cold-start tests and measure loader invocation count and readiness latency.

Scope: IndexCacheService and tests. Out of scope: changing index format or persistence.

Done: a burst creates one loader, failure permits a later retry, and existing cold/warm behavior passes.
