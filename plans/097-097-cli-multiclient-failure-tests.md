# Plan 097: Test partial client registration failures
...
`cli/lib/config.js:815-892` updates multiple client configs with separate patched/failed/skipped results. Existing success-heavy tests in `cli/run.test.js:854-1194` omit mid-batch write/read-back/backup failures.

Add injectable filesystem failure scenarios for backup failure, write failure after an earlier client, post-write read-back failure, and stale backup handling. Assert the CLI reports exact partial state and never claims all clients registered.

Scope: CLI config seams and `cli/run.test.js`. Out of scope: changing client adapter formats.

Done: failure-path tests pass under `npm test`; no real user config is touched.
